...
export default function Home({ lastForm }) {
...
export async function getServerSideProps({ params }) {
  let { editId } = params;
  let response = await fetch(`${url_api}forms/edit/${editId}`, {
    method: "GET",
  });
  let lastForm;
  if (response.ok) {
    lastForm = await response.json();
